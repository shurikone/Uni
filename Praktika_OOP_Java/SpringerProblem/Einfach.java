...
public class Einfach{
     ...
     */
    public void einfachBerechnen(int[][] aktuellerZustand, int groesse, int x, int y, int besucht)
    {
        aktuellerZustand[x][y] = besucht;

        if (aktuellerZustand[0][0] != 0 && aktuellerZustand[0][aktuellerZustand.length-1] != 0 &&
                aktuellerZustand[aktuellerZustand.length-1][aktuellerZustand.length-1] != 0 &&
                aktuellerZustand[aktuellerZustand.length-1][0] != 0 ) {
            istMoeglich = true;
            anzahlLoesung++;
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (valid(newX, newY, groesse, aktuellerZustand) && aktuellerZustand[newX][newY] == 0) {
                einfachBerechnen(aktuellerZustand, groesse, newX, newY,
                        besucht + 1);
            }
        }
